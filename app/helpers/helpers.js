...
exports.createAggregateQuery = reqParams => {
    ...
    let innerConj = reqParams.split('(').length > 1 ? reqParams.split('(')[1].split('~')[0] : undefined;
    let finalParams = {};
    let innerParamsFinal = {}

    const innerParams = innerConj
        ? exports.arrayToObj(reqParams.split(`${innerConj}~`)[1].replace(')', '').split('_'))
        : '';
    const params = innerConj
        ? exports.arrayToObj(reqParams.split('~')[1].slice(0, reqParams.split('~')[1].indexOf('(')).split('_'))
